  ...
  methods: {
    async fetchAccountName() {
      ...
      try {
        ...
        const response = await axios.post(`${import.meta.env.VITE_APP_BASE_URL}/api/withdrawals/resolve-account`, {
    ...
    navigateToTransaction() {
      this.$router.push(`/transaction/${this.userId}`);
    },

    async requestWithdrawal() {
      if (this.amount < 500) {
        this.message = "Minimum withdrawal amount is ₦500";
        this.isError = true;
        return;
      }

      if (!this.userId) {
        this.message = "User not logged in. Please log in to withdraw funds.";
        this.isError = true;
        return;
      }
      ...
      try {
        ...
        const response = await axios.post(`${import.meta.env.VITE_APP_BASE_URL}/api/withdrawals/withdraw/send-otp`, {
          userId: this.userId,
        });
    ...
    async verifyOtp() {
      if (!this.userId) {
        this.message = "User not logged in. Please log in to verify OTP.";
        this.isError = true;
      ...
      try {
        ...
        const response = await axios.post(`${import.meta.env.VITE_APP_BASE_URL}/api/withdrawals/withdraw`, {
          userId: this.userId,
          amount: this.amount,
          otp: this.otp,
          account_bank: this.account_bank,
